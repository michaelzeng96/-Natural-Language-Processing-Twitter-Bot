...
#reshape input data to [samples, time steps, features]. each sample is a 100x1 matrix
#and one hot encode the output variables
X = numpy.reshape(dataX, (len(dataX), seq_length, 1))
X = X / float(len(chars)) #normalize each integer to make learning easier, however not one-hot encoded
...
model.add(Dense(y.shape[1],activation='softmax')) #DENSE = standard feedforward layer, every layer connected (dense) 
